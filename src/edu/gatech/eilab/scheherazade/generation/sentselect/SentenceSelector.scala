 ...
 */
class SentenceSelector (evalSentence: Cluster => List[(Sentence,Double)], evalAjacentSent: (Sentence, Cluster) => List[(Sentence,Double)]) {
  ...
  def bestSentenceSequence(clusters: List[Cluster]):List[Sentence] = {
    ...
    {      
      ...
      {        
        val adjSentFit = evalAjacentSent(oldSeq._1.head, nextCluster)
        for (sf <- adjSentFit)
        {
