...
class HMMAssignment2(probs: Array[Array[Double]], transition: Array[Array[Double]], alpha: Array[Double]) {
  ...
  def saveParam() {
    ...
    val pw = new PrintWriter(new BufferedOutputStream(new FileOutputStream("hmm.log")))

    pw.println("*** probs ***")

    for (i <- 0 until probs.length) {
      for (j <- 0 until probs(0).length) {
        pw.print(probs(i)(j))
        pw.print("\t")
      }
      pw.println
    ...
    for (i <- 0 until transition.length) {
      for (j <- 0 until transition(0).length) {
        pw.print(transition(i)(j))
        pw.print("\t")
      }
      pw.println
    }
