 ...
 *  ==> The Gurobi package is unstable...
 */
class GurobiHMMAssignment(probs: Array[Array[Double]], transition: Array[Array[Double]]) {
  ...
  def solve() = {
    ...
    // The EXHAUSTIVE constraint: Each sentence (and the subsequent sentence) belongs to exactly one cluster
    ...
    for (i <- 1 until numSents; j <- 0 until numClusters) {
      ...
      for (jp <- 0 until numClusters-1) {
        val v2 = varMat(i - 1)(jp)(j)
        constraint.addTerm(-1, v2)
      }

      model.addConstr(constraint, GRB.EQUAL, 0.0, "consistency" + i + "c" + j)
    }
