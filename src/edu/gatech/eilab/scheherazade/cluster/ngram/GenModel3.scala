...
/**
 * soft EM. That is, instead of argmax y, use p(y)
 *
 */
object GenModel3 {
  ...
  def train(oldCorpus: NGramCorpus): DenseVector[Int] = {
    ...
    val corpus = performPCA(oldCorpus)
    ...
    val probY = Array.fill[DenseVector[Double]](numSents)(DenseVector.zeros(numClusters))
    ...
    for (iter <- 1 to maxIterations) {
      ...
      for (i <- 0 until numSents) {
        ...
        val average = probY(i).sum / numSents
        val proportion = probY(i).map(_ - average).map(math.exp)
        val sum = proportion.sum
        probY(i) = proportion.map(_ / sum)
        //println(maxProduct)
        //y(i) = maxY
      }
      ...
      if (iter == maxIterations) {
        ...
        return new DenseVector(probY.map(row => row.argmax))
      }
      ...
      (0 until numClusters).par.foreach { i =>
        var count = 0.0
        var dataList = List[(DenseVector[Double], Double)]()
        for (j <- 0 until numSents) {
          if (probY(j)(i) > 0.01) {
            for (k <- 0 until corpus.ngrams(j).size) {
              val textVector = corpus.vectors(corpus.ngrams(j)(k))
              dataList = (textVector, probY(j)(i)) :: dataList
            }
          }
        }
    ...
    new DenseVector(probY.map(row => row.argmax))
  }

  def optimize(data: List[(DenseVector[Double], Double)], start: DenseVector[Double], components: List[Int]): DenseVector[Double] =
    {
      val n = start.size
  ...
  class DiriOptimizable(val data: List[(DenseVector[Double], Double)], start: Array[Double], val components: Array[Int]) extends Optimizable.ByGradientValue {

    def this(data: List[(DenseVector[Double], Double)], start: DenseVector[Double], components: Array[Int]) = this(data, start.toArray, components)
    ...
    def getValue(): Double =
      {
        ...
        data.map(d => Dirichlet(x).logPdf(d._1) * d._2).sum //- subtractee
      }
    ...
    def getValueGradient(buffer: Array[Double]) {
      for (i <- 0 until realDimension) {
        ...
        val prob1 = data.map(d => Dirichlet(x1).logPdf(d._1) * d._2).sum //- subtractee
        val prob2 = data.map(d => Dirichlet(x2).logPdf(d._1) * d._2).sum //- subtractee
